...
const SliderForm = () => {
  ...
  const onSubmit = (values) => {
    const { car, pickUpDate, pickUpDateTime, dropOffDate, dropOffTime } =
      values;
    ...
    const reservationDto = {
      ...
      dropOffDateTime: moment(
        `${values.dropOffDate} ${values.dropOffTime}`
      ).format("MM/DD/YYYY HH:mm:ss"),
    };
    ...
    isVehicleAvaliable(reservationDto).then((resp) => {
      ...
      if (!isAvailable) {
        toast(
          "The car is not avaliable on these days. Please select another one"
        );
        return;
      }

      values.totalPrice = totalPrice; //toplam ucreti merkezi state e yerlestirebilmek icin initial valuesuna ekledik.
